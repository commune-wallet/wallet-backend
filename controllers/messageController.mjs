...
const getMessage = asyncHandler(async (req, res) => {
  ...
  let received = [];
  try {
    sent = await Message.find({ sender: address, coinUnit: coin.symbol });
    received = await Message.find({ receiver: address, coinUnit: coin.symbol });
  } catch (error) {
  ...
  const messages = sent
    .concat(received)
    .sort((a, b) => new Date(b.legacy_date) - new Date(a.legacy_date));
...
const getUsernameFromAddress = async (address) => {
  const user = await User.aggregate([
    ...
    {
      $match: {
        $or: [{ "addressArray.v": { $regex: address, $options: "i" } }],
      },
    },
  ]);
...
const getContactList = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let received = await Message.find(
    ...
    const names = await Promise.all(
      contacts.map((address) => getUsernameFromAddress(address))
    );
    // Union the arrays and sort by date
